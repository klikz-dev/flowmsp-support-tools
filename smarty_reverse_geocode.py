...
def smartystreets_geocode(lat, lon):
    auth_id = "73eed5a6-a281-351f-38c8-330d8295173b"
    auth_token = "bzhlCsgaYFVMYDDKf2gB"
    url = "https://us-reverse-geo.api.smartystreets.com/lookup?auth-id={auth_id}&auth-token={token}".format(
        auth_id=auth_id, token=auth_token)

    url += "&latitude={lat}".format(lat=lat)
    url += "&longitude={lon}".format(lon=lon)
...
if __name__ == "__main__":
    ...
    for recnum, rec in enumerate(r):
        ...
        r = smartystreets_geocode(rec[args.latitude],
                                  rec[args.longitude])
